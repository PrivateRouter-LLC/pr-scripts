...
# Set LED attributes waiting for Internet
...
write_to_log() {
    LOG_FILE="/root/pr-router.log"
    LAST_BOOT_TIME_FILE="/tmp/.last-boot-time"
    if [ ! -f "$LAST_BOOT_TIME_FILE" ] || [ "$(cat "$LAST_BOOT_TIME_FILE")" != "$(uptime -s)" ]; then
        echo "Creating a new log file: $LOG_FILE"
        echo "$(uptime -s)" > "$LAST_BOOT_TIME_FILE"
        echo "$1" > "$LOG_FILE"
    else
        echo "$1" >> "$LOG_FILE"
    fi
